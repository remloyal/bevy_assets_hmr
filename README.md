...
```toml
[dependencies]
bevy_assets_hmr = { path = "crates/bevy_assets_hmr" }
...
use bevy::reflect::TypePath;
use bevy_assets_hmr::{ConfigDiff, HmrSource};
use serde::{Deserialize, Serialize};
use std::collections::HashSet;
...
impl ConfigDiff for LevelAsset {
    fn diff(old: &Self, new: &Self) -> (HashSet<String>, HashSet<String>, HashSet<String>) {
        if old != new {
            (HashSet::new(), HashSet::new(), ["level".to_string()].into_iter().collect())
        } else {
            (HashSet::new(), HashSet::new(), HashSet::new())
        }
    }
...
| `ConfigRefresh<T>` | Message：`{ new_config: T, target_entities, changed_ids, diff_kind }` |
...
7. **自动缓存校验**：每 5 秒自动校验 `HandleEntityCache` 与实际组件的一致性
